...
**Widget**:
A typed tile on a **Dashboard** — metric, text/markdown, image, Embed, weather, or chart. An Agent may bring a Widget's data current but can never change its type, placement, size, or presence; updating requires matching the declared type, so a chart cannot be overwritten into a metric. An Embed is the one exception to data currency: it renders a human-provided HTTPS page, and its URL is set only through the Dashboard UI — no Agent can write it. The exception costs nothing, because an Embed's URL is a pointer rather than content: the page behind it is already self-updating, so an Agent write would buy no currency while moving configuration — which Users own — into Agent hands. An Embed's frame never shares the application's origin, whatever URL it holds — the isolation is a property of the Embed rather than of the URL, so nothing has to reason about where a URL points or where it redirects to; the cost is that an embedded page gets none of its own cookies or storage.
...
**Memory**:
A persisted summary of prior activity, retrieved per-User per-Workspace and rendered into the **System prompt** on **every** Chat turn once the Workspace has a memory-extraction Provider set — not gated on the Agent's Tool sets. Present on every turn but not retrieved on every turn: a **Chat** pins the rendered block and reuses it verbatim, re-taking it only once the gap since the previous turn passes the re-pin horizon, so the **System prompt** stays byte-identical while the Chat is active and the Provider's cached prefix survives the **Transcript** behind it (ADR-0020). A headless **Trigger** composes no block at all unless it opts in: `includeMemories` on the Trigger row defaults to false, and with it off the retrieval is skipped rather than performed and discarded. Opted in, the Trigger carries no Chat identity and so renders the current block, anchored to its own resolution moment rather than a clock read at composition time. The default is off because a firing's **System prompt** must not drift with interactive-chat activity unrelated to it — the ADR-0020 stability property applied to a Trigger rather than to a Chat. The `memory` Tool set is a separate, additive thing: it grants `memorySearch` / `memoryGet` for reaching beyond the summaries already in the prompt — and, because the pinned block is fixed for the burst, it is the only way a live Chat reaches a summary written after it started.
_Avoid_: memory retrieval per turn (in a Chat the fragment renders every turn; the retrieval does not).
